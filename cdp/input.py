...
class TouchPoint:
    ...
    def from_json(cls, json: T_JSON_DICT) -> 'TouchPoint':
        return cls(
            ...
            id=float(json['id']) if 'id' in json else None,
        )
...
def dispatch_key_event(
    ...
    :param unmodified_text: Text that would have been generated by the keyboard if no modifiers were pressed (except for shift). Useful for shortcut (accelerator) key handling (default: "").
...
def synthesize_pinch_gesture(
    ...
    :param gesture_source_type: Which type of input events to be generated (default: 'default', which queries the platform for the preferred input type).
    '''
    params: T_JSON_DICT = dict()
    params['x'] = x
    params['y'] = y
...
def synthesize_scroll_gesture(
    ...
    :param x_overscroll: The number of additional pixels to scroll back along the X axis, in addition to the given distance.
    :param y_overscroll: The number of additional pixels to scroll back along the Y axis, in addition to the given distance.
