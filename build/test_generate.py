...
def test_cdp_domain():
        ...
        "types": [
            ...
            },
            {
                ...
                "properties": [
                    {
                        "name": "block",
                        "$ref": "Parts.EngineBlock"
                    },
                    {
                        "name": "pistons",
                        "type": "array",
                        "items": {"$ref": "Parts.Piston"},
                    },
                ]
            }
        ],
    ...
    expected = dedent("""\
        ...
        class Wheels(list):
            def to_json(self) -> typing.List['vehicle.Wheel']:
                return self

            @classmethod
            def from_json(cls, json: typing.List['vehicle.Wheel']) -> 'Wheels':
                return cls(json)

            def __repr__(self):
                return 'Wheels({})'.format(super().__repr__())


        @dataclass
        class Engine:
            block: 'parts.EngineBlock'

            pistons: typing.List['parts.Piston']

            def to_json(self) -> T_JSON_DICT:
                json: T_JSON_DICT = dict()
                json['block'] = self.block.to_json()
                json['pistons'] = [i.to_json() for i in self.pistons]
                return json

            @classmethod
            def from_json(cls, json: T_JSON_DICT) -> 'Engine':
                return cls(
                    block=parts.EngineBlock.from_json(json['block']),
                    pistons=[parts.Piston.from_json(i) for i in json['pistons']],
                )
        """) # A domain should have a new line at the end.

    domain = CdpDomain.from_json(json_domain)
    actual = domain.generate_code()
    print('EXPECTED:', expected)
    print('ACTUAL:', actual)
    assert expected == actual
